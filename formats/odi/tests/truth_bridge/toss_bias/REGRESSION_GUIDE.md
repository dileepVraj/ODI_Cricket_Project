...
**Legacy Source:** `formats/odi/tests/regression_suite/analyze_toss_bias/fixtures/analyze_toss_bias_expected_results.json`

## 1. Suite Structure
- **Target Function**: `TeamEngine.analyze_venue_bias`
- **Output Schema**: Nested Dictionary (Legacy "Fixture" Format)
- **Key Discovery**: Iterates through `config/shared/venues.py` (`VENUE_MAP`).
