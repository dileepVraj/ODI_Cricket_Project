...
1. **TASK-163:** Register venue_bias_card in manifest output_types; update venue_bias function entry output_type and discover_bullets to reflect enriched payload.
2. **TASK-162:** Wire enrichment helpers into _build_bias_report — VenueBiasReport now returns all 6 enriched fields end-to-end
3. **TASK-161:** Add 7 venue bias enrichment helpers to venue_calculator.py (Wilson CI, sample reliability, score stats/distribution/extremes, bias trend, toss intelligence)
4. **TASK-160:** VenueBiasReport extended with 6 enrichment TypedDicts (CI, score stats/distribution/extremes, bias trend, toss intelligence) — Phase A of analyze_venue_bias overhaul.
5. **TASK-159:** Player Matchups complete redesign — Concept C Dossier layout: batter chips bar, 2-col card grid, all 47 backend fields surfaced (phase mini-tables, innings splits, confidence blocks, dismissal breakdown, venue badge).
