...
1. **TASK-121:** Matrix-report decision denominators are now defined explicitly as wins plus losses, with silent dirty-winner diagnostics kept local to `ReportBuilder`.
2. **TASK-119R:** MatrixTable applies fixed widths and injects home-team CSS variables for aggregate Match Audit coloring.
3. **TASK-117/120:** Matrix-table OVERALL rows now carry `home_team_color` for frontend jersey resolution.
4. **TASK-115:** Vantage branding and shield icons fully deployed across AppShell and Metadata.
5. **TASK-114:** `global_h2h_report` manifest path added with structured calculator engine.
