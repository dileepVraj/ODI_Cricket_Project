...
## PART 6: THE DO-NOT-TOUCH REGISTRY

The following components have been recently stabilised and MUST NOT be modified without an explicit architect instruction and a full impact trace. Any agent that touches these files without explicit approval has violated this standard.

| Component | File | Reason |
|---|---|---|
| **Core Calculators** | `core/calculators/` (all files) | Recently refactored in Phase 11.3. Clean, typed, and passing compliance. Any change risks reintroducing resolved bugs. |
| **Data Access Layer** | `core/data_access.py` | The most complex and highest-risk file. Handles venue resolution, team hydration, and integrity validation. Changes here corrupt every downstream output. |
| **Type Contract System** | `core/interfaces/team_types.py` | Load-bearing column of the type system. Adding new types is safe. Removing or renaming existing types breaks engines, services, and serializers simultaneously. |
| **Backend Manifest** | `formats/odi/manifest.py` | Single source of truth for the entire frontend. The compliance bouncer's Zero-Literal rule validates all string literals against it. Changes have cascading effects. |
| **Serializer** | `api/serializers.py` | Small, complete, handles every known edge case. There is no reason to touch it. |
| **Format Analyzer** | `api/engine_pool.py` (FormatAnalyzer class) | Newly rebuilt to replace the deleted CricketAnalyzer facade. Let it stabilise before modification. |
