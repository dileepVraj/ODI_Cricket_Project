...
// TODO: move link definitions to manifest navigation config
// when that slot is available
export default function QuickLinks({ links }: { links: QuickLinkItem[] }) {
    ...
    const resolveHref = (href: string): string => {
        ...
        const [, dynamicSegment, ...rest] = href.split("/");
        if (!dynamicSegment?.startsWith(":")) return href;

        const suffix = rest.length > 0 ? `/${rest.join("/")}` : "/";
        if (!activeFormat) return suffix;

        return `/${encodeURIComponent(activeFormat)}${suffix}`;
    };
