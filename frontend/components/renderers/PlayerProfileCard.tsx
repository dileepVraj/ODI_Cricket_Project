...
export default function PlayerProfileCard({ data }: PlayerProfileCardProps) {
    ...
    const intelHeaderMsg = venueLabel && yearsInput ? `${name} at ${venueLabel} - Last ${yearsInput} Years` : venueLabel ? `${name} at ${venueLabel} - All Time` : yearsInput ? `${name} - Last ${yearsInput} Years` : `${name} - Overall`;
        ...
        <div className="[display:flex] [flex-direction:column] [gap:20px]">
            ...
            {view === "bowling_intel" && (
                <>
                    ...
                    <BowlingIntelPanel last10Bowling={last10Bowling} phaseBowling={phaseBowling} />
                </>
            )}
            {view !== "batting_intel" && view !== "bowling_intel" && (
                <>
...
function StatSection({ title, icon, stats, tone }: { title: string; icon: ReactNode; stats: [string, unknown][]; tone: SectionTone; }) {
    return (
        <div>
            <div className="[display:flex] [align-items:center] [gap:8px] [margin-bottom:10px]"><div className={`[width:4px] [height:16px] [border-radius:2px] ${toneBarClass(tone)}`} /><span className="[font-size:0.78rem] [font-weight:700] [color:var(--text-secondary)] [text-transform:uppercase] [letter-spacing:0.06em] [display:inline-flex] [align-items:center] [gap:6px]">{icon} {title}</span></div>
            <div className="[display:grid] [grid-template-columns:repeat(auto-fill,_minmax(140px,_1fr))] [gap:8px]">
                {stats.map(([key, value]) => <div key={key} className="[padding:12px_14px] [background:var(--bg-elevated)] [border-radius:var(--radius-md)] [border:1px_solid_var(--border-subtle)]"><div className="[font-size:0.65rem] [text-transform:uppercase] [letter-spacing:0.05em] [color:var(--text-disabled)] [font-weight:600] [margin-bottom:3px]">{key.replace(/_/g, " ")}</div><div className="[font-size:1.1rem] [font-weight:700] [color:var(--text-primary)] font-numeric">{value === null || value === undefined ? "-" : String(value)}</div></div>)}
            </div>
        </div>
    );
}
...
function BowlingIntelPanel({ last10Bowling, phaseBowling }: { last10Bowling: Array<number | null>; phaseBowling: DataRow[]; }) {
                ...
                <div>
                    ...
                    <div className="[overflow-x:auto]">
                        <table className="[width:100%] [border-collapse:collapse] [font-size:0.82rem]">
                            ...
                            <tbody>
                                {orderedPhaseBowling.map((row) => <tr key={`bowl-${labelText(rowValue(row, "phase"))}`} className="[border-bottom:1px_solid_var(--border-subtle)] last:[border-bottom:none]"><td className="[padding:10px] [text-align:left] [font-weight:600] [color:var(--text-primary)]">{PHASE_LABELS[labelText(rowValue(row, "phase")).toLowerCase()] ?? labelText(rowValue(row, "phase"))}</td>{BOWL_METRIC_COLUMNS.map((column) => <td key={column.key} className="[padding:10px] [text-align:right] [color:var(--text-primary)] font-numeric">{metricText(rowValue(row, column.key))}</td>)}</tr>)}
                            </tbody>
                        </table>
                    </div>
                </div>
