...
import { type SectionTone, labelText, metricText, numericValue, rowValue, toneBarClass } from "@/lib/player-intel";
import { type PlayerPayloadFragment, toPlayerPayloadFragment } from "@/lib/types";
...
function pickStats(obj: PlayerPayloadFragment | null, keys: string[]): [string, unknown][] {
    return !obj ? [] : keys.filter((key) => key in obj).map((key) => [key, obj[key]]);
}

function contextToStats(ctx: PlayerPayloadFragment | null): [string, unknown][] {
    if (!ctx) return [];
...
function formatStatValue(value: unknown): string {
    const scalar = rowValue({ value }, "value");
    if (scalar === undefined) return "-";
    if (typeof scalar === "number") return metricText(scalar);
    const label = labelText(scalar);
    if (label === "") return "-";
    return numericValue(label) === null ? label : metricText(label);
}

export default function PlayerProfileCard({ data }: PlayerProfileCardProps) {
    const { activeFormat, manifest } = useAppContext();
    const playerScoutLinks: QuickLink[] = !manifest ? [] : manifest.categories.find((category) => category.key === "player_scout")?.quick_links ?? [];
...
function StatSection({ title, icon, stats, tone }: { title: string; icon: ReactNode; stats: [string, unknown][]; tone: SectionTone }) {
    return (
        <div>
            ...
            <div className="[display:grid] [grid-template-columns:repeat(auto-fill,_minmax(140px,_1fr))] [gap:8px]">{stats.map(([key, value]) => <div key={key} className="[padding:12px_14px] [background:var(--bg-elevated)] [border-radius:var(--radius-md)] [border:1px_solid_var(--border-subtle)]"><div className="[font-size:0.65rem] [text-transform:uppercase] [letter-spacing:0.05em] [color:var(--text-disabled)] [font-weight:600] [margin-bottom:3px]">{key.replace(/_/g, " ")}</div><div className="[font-size:1.1rem] [font-weight:700] [color:var(--text-primary)] font-numeric">{formatStatValue(value)}</div></div>)}</div>
        </div>
    );
}
