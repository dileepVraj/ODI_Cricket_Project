...
# 2. Formats
test("Format Discovery", f"{BASE}/api/formats")

# 3. Manifest
code, data = test("ODI Manifest", f"{BASE}/api/odi/manifest")
if data:
...
# 4. Context: Teams
code, data = test("Teams List", f"{BASE}/api/odi/context/teams")
if data:
    print(f"  🏏 {len(data.get('teams', []))} teams")

# 5. Context: Venues
code, data = test("Venues List", f"{BASE}/api/odi/context/venues")
if data:
    print(f"  🏟️ {len(data.get('venues', []))} venues")

# 6. Context: Players (India)
code, data = test("Players (India)", f"{BASE}/api/odi/context/players/India")
if data:
    print(f"  👤 {len(data.get('players', []))} players")

# 7. Context: Regions
test("Regions", f"{BASE}/api/odi/context/regions")

# 8. Execute: venue_bias
test("Execute: venue_bias", f"{BASE}/api/odi/execute/venue_bias", "POST", {
    "params": {"venue": "IND_MUMBAI_WANKHEDE", "years": 5}
})

# 9. Execute: global_h2h
test("Execute: global_h2h", f"{BASE}/api/odi/execute/global_h2h", "POST", {
    "params": {"team_a": "India", "team_b": "Australia", "years": 5}
})

# 10. Execute: team_form
test("Execute: team_form", f"{BASE}/api/odi/execute/team_form", "POST", {
    "params": {"team_a": "India", "years": 3}
})

# 11. Error: bad format (EXPECTED 404)
test("Error: Bad format (expect 404)", f"{BASE}/api/xyz/manifest", expected_status=404)

# 12. Error: bad function key (EXPECTED 404)
test("Error: Bad function (expect 404)", f"{BASE}/api/odi/execute/nonexistent", "POST", {
    "params": {}
}, expected_status=404)
