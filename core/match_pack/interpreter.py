...
class MatchInterpreter:
    ...
    def __init__(
        ...
        if rankings is None or bowler_styles is None or player_roles is None:
            try:
                players_module = importlib.import_module(f"formats.{format_key}.config.players")
                rankings_module = importlib.import_module(f"formats.{format_key}.config.rankings")
                ranking_candidates = [
                    "RANKINGS",
