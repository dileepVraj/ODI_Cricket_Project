...
def _extract_value(data_list, index):
...
def transform_h2h_slim(raw_list, home_team, away_team):
    """
...
def transform_h2h_report(raw_list, home_team, away_team):
    """
...
def transform_venue_bias(raw_dict):
    """
...
def transform_team_form(raw_dict, team_name):
    """
...
def transform_dominance_matrix(raw_list, team_name):
    """
    Converts the matrix report from analyze_home_dominance / analyze_away_performance
    into aggregated stats.

    Engine returns keys: 'Opponent', 'Mat', 'Won', 'Lost', 'Tie/NR', 'Win %', 'Last 5'
...
def transform_squad_comparison(comparison_payload):
    """
...
def transform_player_stats(stats_dict):
    """
